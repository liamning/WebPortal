...
public class File
{
    ...
    public string UpdateFile(FileInfo fileInfo, DirectoryInfo tmpDirectory, byte[] fileContent)
    {
        this.dbAccess.open(); 
        Dictionary<string, object> dict;  
        this.dbAccess.open();
        try
        {
            //new directory name checking 
            string sql_diretory_checking;
            string directoryCount;

            //1. cannot modify the parent folder
            if (tmpDirectory.FullName != tmpDirectory.Name)
            {
                sql_diretory_checking = "select COUNT(*) from Directory child "
                                        + "where exists ( "
                                        + "select 1 from Directory parent  "
                                        + "join Path on Path.DirectoryID = parent.ID  "
                                        + "and Path.ID = child.ID "
                                        + "and Path.Type = 'D' "
                                        + "and parent.FullName = @FullName) ";
                string parentFolderName = tmpDirectory.FullName.Replace("/" + tmpDirectory.Name, ""); 
                dict = new Dictionary<string, object>();
                dict.Add("@FullName", parentFolderName);
                directoryCount = this.dbAccess.select(sql_diretory_checking, dict).Rows[0][0].ToString();
                if (directoryCount != "1")
                {
                    return "{\"error\": \"Cannot change the parent directory name.\" }";
                }

            }
            //2. duplicated checking
            sql_diretory_checking = "select count(*) from Directory where FullName = @FullName and ID<>@ID";
            dict = new Dictionary<string, object>();
            dict.Add("@FullName", tmpDirectory.FullName);
            dict.Add("@ID", tmpDirectory.ID);
            directoryCount = this.dbAccess.select(sql_diretory_checking, dict).Rows[0][0].ToString();
            if (directoryCount == "1")
            {
                return "{\"error\": \"Duplicate directory name.\" }";
            }

            this.dbAccess.BeginTransaction();
            //update directories name  
            string sqlDirectory = "UPDATE [Directory] "
                                + "SET [Name] = @Name "
                                + ",[FullName] = @FullName "
                                + "WHERE ID = @ID ";
            ...
            this.dbAccess.update(sqlDirectory, dict);
            ...
            string sql = "UPDATE [File] "
                        ...
                        + (fileContent != null ? ",[Content] = @Content ,[OriginalName] = @OriginalName " : "")
                        + "WHERE ID=@ID ";
            ...
            dict.Add("@ID", fileInfo.ID);  

            this.dbAccess.update(sql, dict);

            this.updateDirectoryFullName();
    ...
    private void updateDirectoryFullName()
    {
        //new directory name checking 
        string sql = "update child "
                    + "set FullName = parent.FullName + '/' + child.Name "
                    + "from Directory child "
                    + "join Path on Path.ID = child.ID and  Path.Type = 'D' "
                    + "join Directory parent on Path.DirectoryID = parent.ID ";
